...
- advantage
  + calls to **get and set** take constant time(因为是像数组一样存下来的)
- disadvantage
...
每当执行Add、AddRange、Insert、InsertRange等添加元素的方法，都会检查内部数组的容量是否不够了，如果是，它就会以当前容量的两倍来重新构建一个数组，将旧元素Copy到新数组中，然后丢弃旧数组，在这个临界点的扩容操作，应该来说是比较影响效率的。（arraylist就是一个可以加倍扩容的object array）
...
## collisions
  ...
  + probing(在原有hash function的基础上加一个probing function f(i))
